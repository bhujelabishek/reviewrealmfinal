...
<body>
    <nav>
        ...
        border-bottom-left-radius: 200px; border-bottom-right-radius: 200px; border-bottom: white;">
            <h1>Write Reveiw On Places</h1>
        </div>
        <div class="books">
            <form action="places.php" method="post" class="forms">
                <label for="textarea"><h1></h1></label><br>
                <div class="newme"><label for="textarea">Place Name:</h1></label><br><br>
                <input type="text" id="name" name="place"><br><br></div>
...
<div class="review">
      <?php
            include "db.php";
            $sql = mysqli_query($conn, "SELECT `email`, `bookname`, `reviews`, `review_on` FROM `reviews` WHERE `category`='place'");
