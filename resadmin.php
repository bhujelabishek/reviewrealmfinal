...
<head>
    ...
    <link rel="stylesheet" href="css/resadmin.css">
</head>
<body>
    <nav>
        ...
        border-bottom-left-radius: 200px; border-bottom-right-radius: 200px; border-bottom: white;">
            <h1> Reveiw On Restaurants</h1>
        </div>
        <!-- <div class="books">
            <form action="ress.php" method="post" class="forms">
                <label for="textarea"><h1></h1></label><br>
                <div class="newme"><label for="textarea">Restaurant Name:</h1></label><br><br>
                <input type="text" id="name" name="restro"><br><br></div>
...
//     $feet=mysqli_query($conn,"SELECT `bookname` from `reviews` where `email`='$userprofile' and `category`='resturant'");
//     $fet=mysqli_query($conn,"SELECT `reviews` from `reviews` where `email`='$userprofile' and `category`='resturant'");
...
<div class="review">
      <?php
            include "db.php";
            $sql = mysqli_query($conn, "SELECT `email`, `bookname`, `reviews`, `review_on`FROM `reviews` WHERE `category`='resturant'");
